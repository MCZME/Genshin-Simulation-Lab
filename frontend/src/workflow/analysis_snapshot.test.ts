...
it("相同定义产生相同快照，相关变化均改变快照", () => {
  ...
  // 边界会话变化（模拟节点补跑出新批次）同样触发重算。
  const sessionsChanged: WorkflowDefinition = {
    ...base,
    nodes: base.nodes.map((node) =>
      node.id === "sim-1"
        ? { ...node, params: { ...node.params, last_sessions: ["run:2"] } }
        : node,
    ),
  };
  expect(regionAnalysisSnapshot(sessionsChanged, "analysis-1")).not.toBe(
    regionAnalysisSnapshot(base, "analysis-1"),
  );
});
