...
> 状态：正式规范。
> 本文固定 `core/systems/aura_icd/` 元素附着冷却领域的模型与算法：Definition 与 Binding、Record 游标推进、固定窗口时间语义、批量虚拟计划与跨系统边界。
> 最后更新：2026-09-05。
...
没有独立 Resolver、Store 或 Snapshot serializer 文件。`AuraIcdRuntime` 内置 Record 索引、版本、规范化帧和幂等身份，是该领域唯一状态所有者和写入口。
...
`IcdDefinitionRegistry` 在组装时注册 Definition，拒绝重复 key 和缺失引用。组装完成后 Registry 视为只读，不在仿真过程中动态注册或从网络、SQLite 读取 Definition。
...
Aura ICD 决议通过 `AURA_ICD_RESOLVED` 事实审计。Runtime 本身只返回计划与提交回执；协调器在完整跨领域提交后按 interaction order 发布事实。
...
本领域不包含完整角色、敌人和环境 ICD 数据，也不定义攻击触发冷却、Reaction Gate 或结果库 schema。这些是内容与外围契约边界，不是 Aura ICD 状态机的未完成分支；完整角色 ICD 数据属于 content 与资产数据迁移范围。
