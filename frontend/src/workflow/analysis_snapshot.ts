/** 分析区域自动重算快照：节点运行时计划 + 区域节点/连线 + 输入就绪状态 → 稳定指纹。 */

import { buildStageRegionPlan } from "./analysis_runner";
...
/**
 * 分析区域可重算快照指纹：
 * - 节点运行时计划（会话组 + 阶段节点拓扑序与输入）覆盖取数/算子/边界数据源变化；
...
export function regionAnalysisSnapshot(
  ...
  const stagePlan = buildStageRegionPlan(definition, regionId);
