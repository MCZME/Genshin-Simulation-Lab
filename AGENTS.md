...
本文档是仓库级 Codex/AI 协作入口。处理本项目任务时，先阅读本文件，再按任务范围阅读 `docs/` 中的正式文档。详细设计、契约和 Git 工作流不在本文件展开。
...
- 项目已完成旧项目核心能力的重建，进入新功能开发阶段：按产品需求与当前版本游戏资料直接实现和扩展能力。
- 仿真数值和机制行为必须可验证；不要仅凭记忆、旧代码或外部资料单独实现角色、武器、圣遗物或反应细节。
- 旧项目位置：`E:\project\Genshin Damage calculation`。仅作行为线索查阅；旧代码和旧测试不能直接成为新架构契约，旧预期值进入 golden case 前必须复核来源与版本。
