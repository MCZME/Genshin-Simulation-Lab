...
def build_max_hp_affected_buff_keys(
    *,
    definitions: Iterable[BuffDefinition],
    attribute_definitions: AttributeDefinitionRegistry,
    modifier_providers: Iterable[ModifierProvider] = (),
) -> frozenset[str]:
    """计算直接或经依赖图间接影响 stat.hp.max 的 Buff definition_key 集合。

    该集合交给 `BuffMaxHpChangeCoordinator` 判定哪些 Buff 计划需要与
    生命值系统统一同步；拒绝职责由协调器接线替代，组装阶段不再直接拒绝。
    """
    ...
    return frozenset(
        ...
        for definition in definitions
        if any(
            template.target_key in max_hp_upstream for template in definition.attribute_modifiers
        )
    )
