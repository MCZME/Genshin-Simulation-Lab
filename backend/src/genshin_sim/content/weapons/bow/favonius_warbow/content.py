...
from genshin_sim.content.weapons.bow.favonius_warbow.data import (
    FAVONIUS_WARBOW_CONTENT_VERSION,
    FAVONIUS_WARBOW_HANDLER_KEY,
    FAVONIUS_WARBOW_WINDFALL_IMPACT_KEY,
)


def create_favonius_warbow_content_unit(
    request: WeaponContentUnitRequest,
) -> ContentUnit:
    """西风猎弓内容单元工厂：基础属性之外贡献顺风而行钩子。"""
    ...
    return ContentUnit(
        ...
        handler_key=FAVONIUS_WARBOW_HANDLER_KEY,
        version=FAVONIUS_WARBOW_CONTENT_VERSION,
        slot=request.slot,
        event_hooks=(
            FavoniusWindfallHook(
                handler_key=FAVONIUS_WARBOW_HANDLER_KEY,
                impact_key=FAVONIUS_WARBOW_WINDFALL_IMPACT_KEY,
        ...
        metadata={"purpose": "favonius_warbow_windfall"},
    )
