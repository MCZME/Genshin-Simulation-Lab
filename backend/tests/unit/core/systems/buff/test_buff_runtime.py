...
def test_prepare_expiry_returns_plan_equivalent_to_update_frame():
    definition = _definition()
    runtime = _runtime(definition)
    result = runtime.apply(_request("exp:1", definition, frame=100, duration=3))

    assert runtime.prepare_expiry(100) is None
    plan = runtime.prepare_expiry(103)

    assert plan is not None
    assert plan.expected_records[0].instance_ref == result.instance_ref
    assert plan.replacement_records[0].removal_reason is BuffRemovalReason.EXPIRED
    runtime.validate(plan)
    receipt = runtime.commit_prevalidated(plan)
    runtime.publish_committed_facts(receipt)
...
def test_prepare_remove_returns_plan_and_commit_publishes_removal():
    definition = _definition()
    runtime = _runtime(definition)
    result = runtime.apply(_request("rm:1", definition, frame=5, duration=4))

    plan = runtime.prepare_remove(
        RemoveBuffRequest(
            request_id="rm:req:1",
            frame=7,
    ...
    assert plan.removal_results[0].reason is BuffRemovalReason.EXPLICIT
    runtime.validate(plan)
    receipt = runtime.commit_prevalidated(plan)
    runtime.publish_committed_facts(receipt)

    assert (
        runtime.buff_store.require(result.instance_ref).removal_reason is BuffRemovalReason.EXPLICIT
    )
    assert runtime.event_engine.frame_events[-1].event_type is EventType.BUFF_REMOVED


def test_reader_property_reflects_store_activity():
    definition = _definition()
    runtime = _runtime(definition)
    runtime.apply(_request("reader:1", definition, frame=5, duration=4))

    active = runtime.reader.active(6, target_ref=CHARACTER)

    assert len(active) == 1
    assert runtime.reader.active(9) == ()
