...
def test_assembler_accepts_buff_definition_with_dynamic_hp_dependency_via_provider_reads():
    ...
    assert assembled.buff_max_hp_coordinator.max_hp_definition_keys == frozenset(
        {"buff.assembler.private_hp"}
    )
    assert assembled.buff_max_hp_coordinator.buff_port is assembled.buff_runtime
    assert assembled.buff_handler.runtime is assembled.buff_max_hp_coordinator


def test_assembler_accepts_buff_definition_writing_max_hp_directly():
    subject_ref = AttributeSubjectRef.character("character:slot_1")
    ...
    definition = BuffDefinition(
        definition_key="buff.assembler.max_hp_flat",
        mechanic_key="mechanic.assembler.max_hp_flat",
        handler_key="character.buff",
        conflict_key="buff.assembler.max_hp_flat",
        target_kinds=frozenset({AttributeSubjectKind.CHARACTER}),
        application_policy=BuffApplicationPolicy.REFRESH,
        value_refresh_policy=BuffValueRefreshPolicy.REPLACE_LATEST,
        max_stacks=1,
        attribute_modifiers=(
            BuffAttributeModifierTemplate(
                term_key="assembler.max_hp.flat",
                target_key=STAT_HP_MAX,
                stage=ModifierStage.FLAT_ADD,
            ),
        ),
