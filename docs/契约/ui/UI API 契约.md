...
设置持久化在项目配置（`config.toml` 的 `ui` 节），不落在浏览器存储；字段语义由前端定义，后端只做类型校验，界面形态见[画布设计](../../架构/UI/画布设计.md) 11.2。未初始化时与其它端点一致返回 `409`。
...
分析区域的节点运行时端点；SQL 由后端拥有，前端只提交结构化节点执行请求，不传 SQL 文本。节点种类、表达式语言、形状推导与校验规则见[分析系统契约](../分析系统契约.md)。旧整图查询端点 `POST /api/v1/analysis/query` 已由决策 D-063 退役，分析执行唯一路径为节点运行时。
...
- 结果详情中的输入快照与初始快照（含成员/变体标签；前端成员标签链路已随决策 D-029 移除，落地后此处是历史结果变体身份的唯一出口，见[决策 D-029](../../决策/UI与工作流.md)）。
- 资产图像。
