...
@dataclass(frozen=True, slots=True)
class CharacterMaxHpReconcilePlan:
    operation_id: str
    frame: int
    target_ref: AttributeSubjectRef
    old_max_hp: float
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            "expected_current_hp",
            validate_non_negative_health_float(
                self.expected_current_hp,
                "expected_current_hp",
            ),
        )
        if self.result.old_max_hp != self.old_max_hp or self.result.new_max_hp != self.new_max_hp:
            raise HealthValidationError("reconcile 计划与结果的最大生命不一致")
        if self.result.target_ref != self.target_ref or self.result.frame != self.frame:
            raise HealthValidationError("reconcile 计划与结果的主体或帧不一致")


@dataclass(frozen=True, slots=True)
class MaxHpReconcileReceipt:
