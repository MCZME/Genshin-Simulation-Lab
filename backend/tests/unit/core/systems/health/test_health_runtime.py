...
def test_max_hp_reconcile_plan_prepare_validate_commit_and_event():
    runtime, health, events = _runtime(current_hp=600)

    plan = runtime.prepare_max_hp_reconcile(
        CHARACTER_REF,
        old_max_hp=1000,
        new_max_hp=1500,
        frame=4,
        operation_id="op:max-hp:1",
    )
    assert plan.expected_store_version == 0
    assert plan.expected_current_hp == 600
    assert plan.result.hp_after == pytest.approx(900)

    runtime.validate_max_hp_reconcile(plan)
    receipt = runtime.commit_max_hp_reconcile_prevalidated(plan)

    assert receipt.plan is plan
    assert health.current_hp == pytest.approx(900)
    assert runtime.reconcile_events_for(receipt)[0].event_type == (
        EventType.CHARACTER_MAX_HP_CHANGED
    )


def test_max_hp_reconcile_plan_rejects_replayed_operation():
    runtime, _health, _events = _runtime(current_hp=600)
    plan = runtime.prepare_max_hp_reconcile(
        CHARACTER_REF,
        old_max_hp=1000,
        new_max_hp=1500,
    ...
    runtime.validate_max_hp_reconcile(plan)
    runtime.commit_max_hp_reconcile_prevalidated(plan)

    with pytest.raises(HealthPlanConflictError, match="已提交"):
        runtime.validate_max_hp_reconcile(plan)


def test_max_hp_reconcile_plan_rejects_current_hp_conflict_without_commit():
    runtime, health, events = _runtime(current_hp=600)
    plan = runtime.prepare_max_hp_reconcile(
        CHARACTER_REF,
        old_max_hp=1000,
    ...
    health.current_hp = 700

    with pytest.raises(HealthPlanConflictError, match="前值冲突"):
        runtime.validate_max_hp_reconcile(plan)
...
def test_max_hp_reconcile_plan_without_change_commits_without_event():
    runtime, health, events = _runtime(current_hp=600)
    ...
    assert runtime.reconcile_events_for(receipt) == ()
    assert events.frame_events == ()
