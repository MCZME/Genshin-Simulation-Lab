...
class CharacterHealthStore:
    ...
    def validate_damage_plan(self, plan: CharacterDamagePlan) -> None:
    ...
    def commit_damage_prevalidated(self, plan: CharacterDamagePlan) -> None:
    ...
    def validate_reconcile_plan(self, plan: CharacterMaxHpReconcilePlan) -> None:
        if plan.operation_id in self._committed_operations:
            raise HealthPlanConflictError(f"生命计划已提交：{plan.operation_id}")
        if plan.expected_store_version != self._version:
            raise HealthPlanConflictError(
        ...
        health = self.require(plan.target_ref)
        if health.current_hp != plan.expected_current_hp:
            raise HealthPlanConflictError("角色当前生命前值冲突")

    def commit_reconcile_prevalidated(self, plan: CharacterMaxHpReconcilePlan) -> None:
        # 预校验后只执行确定性内存赋值，不再产生业务错误。
        self._health_by_ref[plan.target_ref].current_hp = plan.result.hp_after
        if plan.result.hp_before != plan.result.hp_after:
            self._version += 1
        self._committed_operations.add(plan.operation_id)
