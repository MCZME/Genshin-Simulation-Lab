...
describe("节点运行时计划", () => {
  it("编译取数链：拓扑序、参数透传、视图不进入阶段节点", () => {
    ...
    const plan = buildStageRegionPlan(def, "analysis-1");

    expect(plan.session_ids).toEqual(["a"]);
    expect(plan.nodes.find((node) => node.id === "runs1")?.inputs).toEqual([]);
    expect(plan.nodes.map((node) => node.id)).toEqual(["runs1", "lim1"]);
    expect(plan.nodes[1].params).toEqual({ count: 10 });
    expect(plan.nodes[1].inputs).toEqual(["runs1"]);
  });
  ...
  it("join 按端口名对齐左右输入顺序", () => {
    ...
    const plan = buildStageRegionPlan(def, "analysis-1");
    const joinNode = plan.nodes.find((node) => node.id === "j1");
    expect(joinNode?.inputs).toEqual(["runs1", "ev1"]);
  });

  it("展示配置转发节点不进入阶段节点，其上游表为视图阶段源", () => {
    const def = definition(
      [
        simulation("sim", ["a"]),
        analysisNode("runs1", "fetch", { source: "runs" }),
    ...
    expect(plan.nodes.map((node) => node.id)).toEqual(["runs1"]);
  });
});
