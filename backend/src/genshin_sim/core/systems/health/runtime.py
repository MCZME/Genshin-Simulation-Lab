...
class HealthRuntime:
    ...
    def __init__(
        ...
        self._reconcile_sequence = count()
    ...
    def reconcile_max_hp(
        ...
        plan = self.prepare_max_hp_reconcile(
            character_ref,
            old_max_hp,
            new_max_hp,
            frame,
            operation_id=f"max-hp-reconcile:{next(self._reconcile_sequence)}",
        )
        self.validate_max_hp_reconcile(plan)
        receipt = self.commit_max_hp_reconcile_prevalidated(plan)
        for event in self.reconcile_events_for(receipt):
            self.event_engine.publish(event)
        return plan.result
    ...
    def prepare_max_hp_reconcile(
        ...
        *,
        ...
        return CharacterMaxHpReconcilePlan(
            ...
            new_max_hp=new_max_hp,
            expected_store_version=self.character_health_store.version,
            expected_current_hp=hp_before,
            result=result,
        )

    def validate_max_hp_reconcile(self, plan: CharacterMaxHpReconcilePlan) -> None:
        # 旧最大生命的真值由调用方的生命周期边界视图负责；这里只校验
        # Store 版本与当前生命前值，防止计划在准备与提交之间被干扰。
        self.character_health_store.validate_reconcile_plan(plan)

    def commit_max_hp_reconcile_prevalidated(
        self,
        plan: CharacterMaxHpReconcilePlan,
    ) -> MaxHpReconcileReceipt:
        self.character_health_store.commit_reconcile_prevalidated(plan)
        return MaxHpReconcileReceipt(plan)

    def reconcile_events_for(self, receipt: MaxHpReconcileReceipt) -> tuple[GameEvent, ...]:
        result = receipt.plan.result
        if result.old_max_hp == result.new_max_hp:
            return ()
        return (
            GameEvent(
