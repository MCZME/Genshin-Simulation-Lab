...
describe("app settings", () => {
  ...
  it("生成后端保存负载", () => {
    expect(toSettingsPayload({ runAnimation: false, workspaceDataDir: null })).toEqual({ run_animation: false });
  });

  it("loadAppSettingsFromApi 读取后端设置", async () => {
    vi.stubGlobal("fetch", fetchMock);
    fetchMock.mockResolvedValueOnce(apiResponse({ run_animation: false, workspace: { data_dir: "data" } }));
  ...
  it("saveAppSettingsToApi 调用 PUT /settings", async () => {
    ...
    await saveAppSettingsToApi({ runAnimation: false, workspaceDataDir: null });

    const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit];
    expect(url).toBe("/api/v1/settings");
    expect(init.method).toBe("PUT");
    expect(JSON.parse(String(init.body))).toEqual({ run_animation: false });
  });
});
