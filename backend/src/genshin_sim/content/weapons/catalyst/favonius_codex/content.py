...
from genshin_sim.content.weapons.catalyst.favonius_codex.data import (
    FAVONIUS_CODEX_CONTENT_VERSION,
    FAVONIUS_CODEX_HANDLER_KEY,
    FAVONIUS_CODEX_WINDFALL_IMPACT_KEY,
)


def create_favonius_codex_content_unit(
    request: WeaponContentUnitRequest,
) -> ContentUnit:
    """西风秘典内容单元工厂：基础属性之外贡献顺风而行钩子。"""
    ...
    return ContentUnit(
        ...
        handler_key=FAVONIUS_CODEX_HANDLER_KEY,
        version=FAVONIUS_CODEX_CONTENT_VERSION,
        slot=request.slot,
        event_hooks=(
            FavoniusWindfallHook(
                handler_key=FAVONIUS_CODEX_HANDLER_KEY,
                impact_key=FAVONIUS_CODEX_WINDFALL_IMPACT_KEY,
        ...
        metadata={"purpose": "favonius_codex_windfall"},
    )
