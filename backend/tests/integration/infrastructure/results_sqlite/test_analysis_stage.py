...
def _execute_plan_once(db_path, plan):
    """共享编译核心的一次编译执行：作为节点逐阶段物化路径的对照面。"""
...
def test_stage_runtime_matches_one_shot_for_derive_chain(tmp_path) -> None:
    """新旧执行路径 golden 对齐：常量列链结果完全一致。"""
...
def test_stage_runtime_single_takes_first_row_and_matches_one_shot(tmp_path) -> None:
    """新旧执行路径 golden 对齐：single 输出与一次编译执行一致（≤1 行、形状透传）。"""
